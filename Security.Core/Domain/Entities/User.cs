...
namespace Axity.Security.Core.Domain.Entities
{
    public class User : BaseEntity
    {
        ...
        {
            return _refreshTokens.Any(rt => rt.Active);
        }

        public string GetActiveTokenUser
        {
            get
            {
                return _refreshTokens.Where(rt => rt.UserId == this.Id && rt.Active).Last().Token;
            }
        }
        ...
        public void AddRefreshToken(string token,int userId,string remoteIpAddress,double daysToExpire)
        {
